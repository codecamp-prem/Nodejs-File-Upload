...
Introduction
------------
Recently, I’ve been working on a file upload from a web app, which needed to report the upload progress to the end user. 
I typically use the browsers’ native fetch function to perform API calls to the server. 
It supports promises and does not require external dependencies. For the few browsers, which do not implement fetch, there are convenient polyfills.
...
Thus, I had to resolve to a native browser primitive called XMLHttpRequest. Unfortunately, it’s quite old school and did not fit well with the rest of project’s code which uses promises. In this article, I’ll show how to wrap the XMLHttpRequest primitives in a simple promise-based function for arbitrary form submission.
...
The result of the function call is a promise which resolves with the server response if the form submission was successful. 
On failure the promise rejects/fails. Check out the comments in the code below for more details on the implementation:
*/
